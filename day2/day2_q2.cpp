...
int main (int argc, char** argv) {
    ...
    while (std::getline(inputFileStream, line)) {
        ...
        std::map<std::string, int> maxBallColorCounts;
        maxBallColorCounts["green"] = 0;
        maxBallColorCounts["blue"] = 0;
        maxBallColorCounts["red"] = 0;
        for (int roundId = 0; roundId < gameRounds.size(); ++roundId) {
            ...
            for (int infoId = 0; infoId < ballInfos.size(); ++infoId) {
                ...
                if (maxBallColorCounts[ballColor] < ballCount) {
                    maxBallColorCounts[ballColor] = ballCount;
                }
            }
        }
        double gamePower = maxBallColorCounts["green"] * maxBallColorCounts["blue"] * maxBallColorCounts["red"];
        sum += gamePower;
    }
